...
export class FilterDishPipe implements PipeTransform {
    ...
    private buildFilter(value: string): Prisma.DishWhereInput {
        ...
        for(const cond of conditions) {
            ...
            if(isEmpty(attr) || isEmpty(operator) || isEmpty(condValue))
                throw new BadRequestException(`Bad parameter here : ${cond}`);
            ...
            if(attr === "name" && operator === "contains")
                ...
                if(isNaN(nb) || nb < 0)
                    throw new BadRequestException(`Price should be a positive number : ${condValue}`);
